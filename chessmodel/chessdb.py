...
def download_database():
    ...
    update_database(conn, fen, set())


def update_database(conn, fen, evaluating):
    if fen in evaluating or is_conclude(conn, fen):
        return
    evaluating.add(fen)
    ...
    for next_board in next_boards:
        update_database(conn, next_board, evaluating)
...
def queryboards_fen_imply(fen):
    query = urllib.parse.quote(fen)
    r = requests.get('http://api.chessdb.cn:81/chessdb.php?action=queryall&board=' + query).text.strip('\x00')
