...
import pickle
#import engine as ec
from data import create_feed, unfeed
import ast
...
class SearchEngine:
    ...
    def search(self, board, red, depth):
        ...
        self.boards = []
        self.deep_search(
            board, depth, red, (-rule.GAMEOVER_THRESHOLD*3, rule.GAMEOVER_THRESHOLD*3))
        idx = [i for _, i in sorted(zip(self.scores, range(
            len(self.scores))), key=lambda x: -x[0] if red else x[0])]
    ...
    def deep_search(self, board, depth, red, window):
        ...
        gameover_state = any(
            [abs(x) >= rule.GAMEOVER_THRESHOLD for x in next_scores])
        ...
        if depth == 1 or gameover_state:
            best_move, best_score = max(
                zip(moves, next_scores), key=lambda ms: (ms[1] if red else -ms[1]))
            if depth == self.depth:
        ...
        idx = sorted(idx, key=lambda i: -
                     next_scores[i] if red else next_scores[i])
